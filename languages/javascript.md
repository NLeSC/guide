...
sudo apt-get install npm nodejs nodejs-legacy ruby ruby-dev
sudo gem install compass
npm install -g typescript typings bower nodemon http-server
```
...
```shell
npm install -g yo generator-fountain-webapp bower
```
...
However, most of these generators use either gulp or grunt as a build system, while for most cases npm works just
fine by defining a number of npm scripts in the package.json. See [this blog post](http://substack.net/task_automation_with_npm_run)
...
The easiest way to make sure you have the correct types for your framework is by using the [typings tool](https://github.com/typings/typings),
...
To be able to use its functionality in typescript we need to install the typings. We can search for the correct
package:
```shell
...
### Debugging
Chrome supports debugging of typescript by using so called sourcemaps.
Source maps can be created by invoking the typescript compiler with the -sourcemap flag.
```shell
...
Generating sourcemaps in your build script should be fairly straighforward.
